...
def add_orders():
    ...
    for _ in range(1000):
        ...
        shipped_date = random.choices(
            [None, fake.date_time_between(start_date=ordered_date)], [10, 90])[0]

        # Choose either random or None date for delivered and shipped
        delivered_date = None
        if shipped_date:
            delivered_date = random.choices(
                [None, fake.date_time_between(start_date=shipped_date)], [50, 50])[0]

        # Choose either None or  one of three coupon code
        coupon_code = random.choices(
            [None, '50OFF', 'FREESHIPPING', 'BUYONETAKEONE'], [80, 5, 5, 5])[0]
...
def get_pending_orders():
    ...
    peding_orders = Order.query.filter(Order.shipped_date.is_(None)).all()
    for order in peding_orders:
        print(order.order_date)


def how_many_customers():
    print('how_many_customers')
    print(Customer.query.count())


def orders_with_code():
    print('orders_with_code')
